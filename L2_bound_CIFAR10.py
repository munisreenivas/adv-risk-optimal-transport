...
# The adversarial model is an L2 norm perturbation adversary with budget epsilon.
...
x0 = x_train.data[x0_inds].reshape((n,-1)).astype(float)/255.0
x1 = x_train.data[x1_inds].reshape((n,-1)).astype(float)/255.0


dist0 = pdist(x0, 'euclidean')
dist1 = pdist(x1, 'euclidean')
...
for epid in range(len(ep_range)):
    ...
    for sigid in range(len(sig_range)):
        ...
        opt_cost_sig = 0
        for r,c in zip(rids, cids): 
            mean_diff = np.linalg.norm(x0[r]-x1[c])  
...
plt.xlabel(r'Adversarial budget, $\epsilon$', fontsize = 'xx-large')
plt.ylabel('Adversarial error lower bound', fontsize = 'xx-large')
plt.grid()
plt.show()
plt.savefig('cifar_l2.png', bbox_inches='tight')
